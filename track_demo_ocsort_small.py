...
import pickle
from visualization_new_small import start_visualization_thread
...
CONFIG = {
    # Video Processing
    'video_path': r"E:\users\sjj\小球\ZQ\补\200f7-1-Z.mp4",
    'output_dir': r'D:\pythonproject\yzh\yolo11\1',
    'model_weights': r'D:\pythonproject\yzh\yolo11\yolo11s_small_no_aug\train\weights\best.pt',
    'device': '0',
...
def visualize_area_calculation(
    ...
    white_areas = (binary == 255).astype(np.uint8)
...
def generate_filtered_video(
        ...
        video_path: str,
        original_video_path: str,
        speeds: Dict[int, float] = None  # Add speeds parameter
) -> None:
    """
    生成两种过滤后的视频:
    1. 完整视频：显示所有轨迹、目标ID和速度
    2. 目标视频：在新背景下只显示过滤后的目标
    ...
    print(f"正在生成过滤后的视频,包含 {len(filtered_ids)} 个目标...")
    ...
    print(f"完整轨迹视频已保存到: {video_path}")
    print(f"目标视频已保存到: {target_only_path}")
...
def plot_trajectories(
        ...
        dpi_setting: int = 150  # 优化：降低了 DPI
) -> None:
    """
    优化后的轨迹可视化函数。
    ...
    try:
        ...
        plt.close() # 关闭图形以释放内存
        print(f"已将优化后的轨迹图保存至 {save_path} (DPI: {dpi_setting})")
    ...
    try:
        ...
        while True:
            ...
            if filtered_ids:
                ...
                print("\n=== 视频生成完成 ===")
                print("已生成以下四个版本的视频：")
                print(f"1. 原始完整版本（带ID和近期轨迹）: filtered_video_{timestamp}.mp4")
                print(f"2. 原始目标版本（带ID和近期轨迹）: targets_only_{timestamp}.mp4")
